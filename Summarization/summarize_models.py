#!/usr/bin/env python3
"""Summarize parsed summarization benchmark outputs for each model directory.

This script mirrors ``QnA/summarize_models.py`` but is intended for the
CNN/DailyMail-style summarization task. It walks the immediate sub-directories
of the given root (default: the current working directory), looks for a
``parsed_outputs_*.json`` file, loads it, computes aggregate statistics plus
per-sample time/memory/carbon traces, and writes a ``summary_metrics.json``
file back into the same directory.

The resulting summaries are convenient for plotting cross-model graphs
(latency, throughput, memory, answer length, carbon, etc.).
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize parsed summarization benchmark outputs")
    parser.add_argument(
        "--root",
        default=".",
...
SERIES_SPECS: Dict[str, Callable[[Dict[str, Any]], Optional[float]]] = {
    ...
    "summary_length_chars": lambda e: float(len(e.get("model_answer", ""))),
...
def build_per_sample(entry: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "summary_length_chars": len(entry.get("model_answer", "")),
...
def summarize_entries(entries: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    per_sample = [build_per_sample(entry) for entry in entries]
